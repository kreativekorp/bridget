...
public class BridgetJWS extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Object allUnnamed = getModule.invoke(BridgetJWS.class);
	...
	public BridgetJWS() {
		...
		mbar = new JMenuBar();
		mbar.add(fmenu = new JMenu("File")); fmenu.setMnemonic(KeyEvent.VK_F);
		...
		fmenu.add(miExit = new JMenuItem("Exit")); miExit.setMnemonic(KeyEvent.VK_Q); miExit.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Q, this.getToolkit().getMenuShortcutKeyMask()));
		mbar.add(menu = new JMenu("Options")); menu.setMnemonic(KeyEvent.VK_O);
		...
		mbar.add(hmenu = new JMenu("Help")); hmenu.setMnemonic(KeyEvent.VK_H);
		...
		hmenu.add(miAbout = new JMenuItem("About Bridget")); miAbout.setMnemonic(KeyEvent.VK_A);
		...
		gtw = new Thread() {
			public void run() {
				try {
					Random r = new Random();
					while (true) {
						BridgetBoard bb = new BridgetBoard(BridgetJWS.this.size);
						BridgetGUIPanel bp = new BridgetGUIPanel(bb, BridgetJWS.this.oStarts = !BridgetJWS.this.oStarts, false, new BridgetGUIPlayer(), new BridgetGUIPlayer());
						BridgetJWS.this.setContentPane(bp);
						BridgetJWS.this.pack();
						Thread.sleep(5);
						while (true) {
							BridgetPoint p = new BridgetPoint(1+r.nextInt(BridgetJWS.this.size), 1+r.nextInt(BridgetJWS.this.size));
							while (!bb.isValidMove(p)) {
								p = new BridgetPoint(1+r.nextInt(BridgetJWS.this.size), 1+r.nextInt(BridgetJWS.this.size));
							}
							bp.makeMove(p);
							Thread.sleep(5);
							if (bb.isGameOver()) {
					...
					BridgetGUIPanel bp = (BridgetGUIPanel)BridgetJWS.this.getContentPane();
